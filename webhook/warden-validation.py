...
def k8s_response(allowed, uid, message):
    #mutating
    # json_patch = jsonpatch.JsonPatch([{"op": "add", "path": "/metadata/labels/fiki", "value": "bok"}])
    # base64patch = base64.b64encode(json_patch.to_string().encode('utf-8')).decode('utf-8')
    # return jsonify({"apiVersion": "admission.k8s.io/v1", "kind": "AdmissionReview", "response": {"allowed": allowed, "uid": uid, "status": {"message": message}, "patchType": "JSONPatch", "patch": base64patch}})
    #validation
